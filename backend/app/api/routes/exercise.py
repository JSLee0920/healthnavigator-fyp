...
async def list_logs(
    ...
    stmt = (
        select(ExerciseLog)
        .where(ExerciseLog.user_id == current_user.user_id)
        .order_by(ExerciseLog.logged_at.desc())
        .limit(limit)
    )
    if from_:
        stmt = stmt.where(ExerciseLog.logged_at >= from_)
    if to:
        stmt = stmt.where(ExerciseLog.logged_at <= to)
...
async def get_summary(
    ...
    this_week_minutes = int(minutes_result.scalar() or 0)
